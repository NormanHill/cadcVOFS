...
class URI:
    ...
    def __init__(self, uri):
        self.uri = uri
        self.match = re.match("(^(?P<scheme>[a-zA-Z]*):)?(//(?P<netloc>[^/]*))?(?P<path>/?.*)?", uri)
        if self.match is None:
            raise exceptions.ValueError("malformed uri: {}".format(uri))

    @property
    def scheme(self):
        return self.match.group('scheme')

    @property
    def netloc(self):
        return self.match.group('netloc')

    @property
    def path(self):
        return self.match.group('path')
...
class Connection:
    ...
    def __init__(self, certfile=None, http_debug=False):
        ...
        @param certfile: The filesystem location of the .pem security certificate used for SSL access
        @param http_debug: Should the httplib debugger be turned on?  highly verbose.
        """
    ...
    def get_connection(self, url):
        ...
        @param url: URL to connect to using this connection object.
        """
        parts = URI(url)
        logger.debug("Creating connection to %s using %s" % (str(parts.netloc), self.certfile))
        ...
        try:
            if parts.scheme == "https":
                connection = httplib.HTTPSConnection(parts.netloc,
                                                     ...
                                                     timeout=MAX_RETRY_TIME)
            else:
                connection = httplib.HTTPConnection(parts.netloc, timeout=MAX_RETRY_TIME)
        except httplib.NotConnected as e:
            logger.error("{} connection to {} failed".format(parts.scheme, parts.netloc))
            logger.error("%s \n" % (str(e)))
            raise OSError(errno.ECONNREFUSED, "VOSpace connection failed", parts.netloc)
        ...
        while True:
            try:
                ...
                delay = time.time() - connection_time_start
                if delay > MAX_RETRY_TIME:
                    raise e
                logger.critical("Connection failed for {}s, retrying for {}s".format(delay, MAX_RETRY_TIME))
...
class Node:
    ...
    def __init__(self, node, node_type=None, properties=None, sub_nodes=None, **kwargs):
        ...
        @param node: Either a actual node object or the name of the node to create.
        @param node_type: Whay type of node to create.
        @param properties: A dictionary of properties for this node.
        @param sub_nodes: A list of nodes contained in the given node.
        @param kwargs: something to grab deprecated arguments.
        """
        sub_nodes = (sub_nodes is None and []) or sub_nodes
        properties = (properties is None and {}) or properties
        node_type = (node_type is None and DATA_NODE) or node_type

        if kwargs:
            logger.debug("Deprecated keywords passed to Node constructor. {}".format(kwargs.keys()))

        if type(node) == unicode or type(node) == str:
            node = self.create(node, node_type, properties, subnodes=sub_nodes)
    ...
    def create(self, uri, node_type=None, properties=None, subnodes=None):
        ...
        if not node_type: node_type = DATA_NODE
        ...
        for property in properties.keys():
            propertyNode = ElementTree.SubElement(propertiesNode, Node.PROPERTY)
            propertyNode.attrib['readOnly'] = "false"
            ### There should be a '#' in there someplace...
        ...
        ElementTree.SubElement(accepts, "view").attrib['uri'] = "%s#%s" % (Node.IVOAURL, "defaultview")

        ### create provides section
        provides = ElementTree.SubElement(node, Node.PROVIDES)
        ElementTree.SubElement(provides, "view").attrib['uri'] = "%s#%s" % (Node.IVOAURL, 'defaultview')
        ElementTree.SubElement(provides, "view").attrib['uri'] = "%s#%s" % (Node.CADCURL, 'rssview')

        ### Only DataNode can have a dataview...
        if node_type == DATA_NODE:
            ElementTree.SubElement(provides, "view").attrib['uri'] = "%s#%s" % (Node.CADCURL, 'dataview')

        ### if this is a container node then we need to add an empy directory contents area...
        if node_type == CONTAINER_NODE:
...
class VOFile:
    ...
    def read(self, size=None):
        ...
        if not self.closed:
            try:
                logger.debug("getting the connection response")
                if self.transEncode is not None:
                    self.httpCon.send('0\r\n\r\n')
                self.resp = self.httpCon.getresponse()
            except Exception as e:
            ...
            self.closed = True
        self.checkstatus()
        logger.debug("Reading the bytes from the connection.")
        errnos = {404: errno.ENOENT,
                  401: errno.EACCES,
                  409: errno.EEXIST,
                  408: errno.EAGAIN}
        ...
        if self.resp.status == 200:
            ...
            logger.debug("sending back {} bytes.".format(len(buff)))
            return buff
...
class Client:
    ...
    def copy(self, src, dest, sendMD5=False):
        ...
        if src[0:4] == "vos:":
            ...
            srcMD5 = ((srcSize == 0 and "d41d8cd98f00b204e9800998ecf8427e")
                      or srcNode.props.get["MD5"])
        ...
        if checkSource:
            if srcNode.type != "vos:LinkNode":
                ...
                # TODO
                # this is a hack .. 
                # we should check the data integrity of links too
                # just not sure how
                checkMD5 = md5.hexdigest()
        else:
            checkMD5 = ((destSize == 0 and "d41d8cd98f00b204e9800998ecf8427e")
                        or self.getNode(dest, force=True).props["MD5"])
    ...
    def getNode(self, uri, limit=0, force=False):
        ...
        if force or uri not in self.nodeCache:
            vofile = self.open(uri, os.O_RDONLY, limit=limit)
            logger.debug("file is closed? {}".format(vofile.closed))
            xml_file = StringIO(vofile.read())
            xml_file.seek(0)
            try:
                dom = ElementTree.parse(xml_file)
            except Exception as e:
                logger.error("Failed while making DOM from XML response.")
                raise e
            xml_file.close()
            logger.debug("closing the connection.")
            #response = vofile.close()
            #logger.debug("Back from close. {} ".format(response))
            node = Node(dom.getroot())
            logger.debug("Building node: {}".format(node.name))
            ...
            if limit != 0 and node.isdir() and len(node.getNodeList()) > 500:
                ...
                while nextURI != node.getNodeList()[-1].uri:
                    ...
                    vofile = self.open(uri, os.O_RDONLY, limit=limit, nextURI=nextURI)
                    xml_file = StringIO(vofile.read())
                    xml_file.seek(0)
    ...
    def getNodeURL(self, uri, method='GET', view=None, limit=0, nextURI=None, cutout=None):
        ...
        if view in ['cutout', 'data']:
            ...
            URL = "{}://{}{}?{}".format(self.protocol, server, self.VOTransfer, query)
    ...
    def link(self, srcURI, linkURI):
        ...
        ElementTree.SubElement(linkNode.node, "target").text = self.fixURI(srcURI)
    ...
    def move(self, srcURI, destURI):
        ...
        ElementTree.SubElement(transfer, "direction").text = self.fixURI(destURI)
    ...
    def transfer(self, uri, direction):
        ...
        transferXML = ElementTree.Element("transfer")
        transferXML.attrib['xmlns'] = Node.VOSNS
        transferXML.attrib['xmlns:vos'] = Node.VOSNS
        ElementTree.SubElement(transferXML, "target").text = uri
        ElementTree.SubElement(transferXML, "direction").text = direction
        ElementTree.SubElement(transferXML, "view").attrib['uri'] = "%s#%s" % (Node.IVOAURL, "defaultview")
        ElementTree.SubElement(transferXML, "protocol").attrib['uri'] = "%s#%s" % (Node.IVOAURL, protocol[direction])
        url = "%s://%s%s" % (self.protocol, SERVER, Client.VOTransfer)
        con = VOFile(url, self.conn, method="POST", followRedirect=False)
        con.write(ElementTree.tostring(transferXML))
        transURL = con.read()
        logger.debug("Got back %s from trasnfer " % (con))
        con = StringIO(VOFile(transURL, self.conn, method="GET", followRedirect=True).read())
        con.seek(0)
        F = ElementTree.parse(con)
